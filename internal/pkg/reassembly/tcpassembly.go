...
type page struct {
	...
	continuationContext AssemblerContext // retained on subsequent pages for metadata and flushes
	seen                time.Time
	start, end          bool
}
...
func (p *page) captureInfo() gopacket.CaptureInfo {
	return p.assemblerContext().GetCaptureInfo()
}
...
func (lp *livePacket) convertToPages(pc *pageCache, skip int, ac AssemblerContext) (*page, *page, int) {
	...
	for {
		...
		current.ac = nil
		current.continuationContext = ac
		numPages++
	}
