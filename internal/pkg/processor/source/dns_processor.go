...
// dnsProcessorImpl implements DNSProcessor using dns.Parser and dns.TunnelingDetector.
type dnsProcessorImpl struct {
	parser    *dns.Parser
	tunneling *dns.TunnelingDetector
}

// NewDNSProcessor creates a new DNS processor for tap-side analysis.
...
func (p *dnsProcessorImpl) ProcessPacket(packet gopacket.Packet) *data.DNSMetadata {
	if packet == nil {
		return nil
	}
