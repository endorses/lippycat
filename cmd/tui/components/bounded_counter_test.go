...
func TestBoundedCounter_BasicIncrement(t *testing.T) {
	...
	assert.Equal(t, 2, bc.Get("foo"))
	assert.Equal(t, 1, bc.Get("bar"))
	assert.Equal(t, 0, bc.Get("nonexistent"))
...
func TestBoundedCounter_CapacityLimit(t *testing.T) {
	...
	assert.Equal(t, 1, bc.Get("d"), "new item should be added")
}
...
func TestBoundedCounter_EvictsLowestCount(t *testing.T) {
	...
	assert.Equal(t, 0, bc.Get("low"), "lowest count item should be evicted")
	assert.Equal(t, 3, bc.Get("high"))
	assert.Equal(t, 2, bc.Get("medium"))
	assert.Equal(t, 1, bc.Get("new"))
}
...
func TestBoundedCounter_IncrementExistingDoesNotEvict(t *testing.T) {
	...
	assert.Equal(t, 3, bc.Len())
	assert.Equal(t, 3, bc.Get("a"))
	assert.Equal(t, 1, bc.Get("b"))
	assert.Equal(t, 1, bc.Get("c"))
}
...
func TestBoundedCounter_GetAll(t *testing.T) {
	...
	assert.Equal(t, 2, all["foo"])
	assert.Equal(t, 1, all["bar"])
...
func TestBoundedCounter_GetTopN(t *testing.T) {
	...
	assert.Equal(t, 10, top2[0].Count)
	assert.Equal(t, "medium", top2[1].Key)
	assert.Equal(t, 5, top2[1].Count)
...
func TestBoundedCounter_Clear(t *testing.T) {
	...
	assert.Equal(t, 0, bc.Get("foo"))
	assert.Equal(t, 0, bc.Get("bar"))
}
...
func TestBoundedCounter_MaintainsTopItems(t *testing.T) {
	...
	assert.Equal(t, 100, topItems[0].Count)
	assert.Equal(t, "B", topItems[1].Key)
	assert.Equal(t, 95, topItems[1].Count)
}
...
func TestBoundedCounter_SingleCapacity(t *testing.T) {
	...
	assert.Equal(t, 1, bc.Get("first"))

	// Add second item - should replace first
	bc.Increment("second")
	assert.Equal(t, 1, bc.Len())
	assert.Equal(t, 0, bc.Get("first"))
	assert.Equal(t, 1, bc.Get("second"))
}
