...
func runEmailTap(cmd *cobra.Command, args []string) error {
	...
	runtime, err := newTapRuntime(config, effectiveBPFFilter, tapRuntimeAdapter{protocol: protocolcatalog.MustLookup("email")})
	if err != nil {
		return err
	}
	...
	return runtime.run("Email Tap node", config)
}
