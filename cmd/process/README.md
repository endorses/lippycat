...
After a call has finalized, reuse of its Call-ID starts a new generation only with a distinct INVITE transaction. Media waits for an answered INVITE and a newly advertised SDP port. Reused prior-generation SDP ports, observed RTP port pairs, and observed SSRCs are conservatively rejected, even when legitimate; post-restart ACK/BYE requires matching dialog tags. These rejections are counted in processor SIP retry telemetry. The writer idle timeout cannot close a matched-503 call before its retry deadline.
