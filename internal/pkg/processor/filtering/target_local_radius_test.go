...
func TestLocalTargetRADIUSRevisionReconcilesPolicy(t *testing.T) {
	target := NewLocalTarget(LocalTargetConfig{BaseBPF: "tcp port 80"})
	target.SetBPFUpdater(&radiusBPFUpdater{ports: []uint16{19120}})
	target.SetApplicationFilter(&radiusAppUpdater{})
	coordinator := &recordingLocalFilterCoordinator{}
	target.SetCoordinator(coordinator)
	filter := &management.Filter{Id: "radius", Type: management.FilterType_FILTER_RADIUS_USERNAME, Pattern: "alice", Enabled: true, Revision: 1}
	...
	require.Equal(t, uint64(2), changes[1].Next.ApplicationFilters[0].Revision)
	require.Contains(t, changes[1].Next.BPFExpression, "19120")
	require.Equal(t, changes[0].Next.BPFExpression, changes[1].Next.BPFExpression)
}
