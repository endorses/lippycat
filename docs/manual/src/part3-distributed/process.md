...
even when total spool storage is unlimited. The receiver's
`--event-ingress-max-batch-bytes` must therefore be at least 4 MiB, and
`--event-queue-size` must accommodate a complete incoming batch (up to 4,096
events). For recovery behavior and the safe response to startup or durability
errors, see
