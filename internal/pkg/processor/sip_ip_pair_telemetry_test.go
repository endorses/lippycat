...
func TestVirtualHunterDetectorLifecycle(t *testing.T) {
	...
	p := &Processor{
		...
		config:               Config{ProcessorID: "tap", EnableDetection: false},
		sessionOutputManager: newSessionOutputManager(nil, nil),
	}
	for range 3 {
		got := p.SynthesizeVirtualHunter().GetStats().GetDetector()
