...
```bash
GOCACHE=/tmp/lippycat-go-cache go run ./testdata/radius
GOCACHE=/tmp/lippycat-go-cache go run ./testdata/radius --check
GOCACHE=/tmp/lippycat-go-cache go test ./testdata/radius
```
...
including its original Authenticator. These are **payload goldens for later X2
...
The standard-library-only checker independently parses committed PCAP/IP/UDP
...
line scope and response associations. A separate reproducibility check compares
generated artifacts with committed bytes. Production decoder, task admission,
timeout/ambiguity behavior, and MDF interoperability require later-phase tests.
Because Go excludes `testdata` from `./...` expansion, run this package explicitly.
