...
Equivalent YAML keys are `events.queue_size`, `logs.dir`, `logs.format`,
`logs.streams`, `logs.rotate_interval`, `logs.queue_size`, `logs.emit_stage`,
`logs.post_rotate_command`, `logs.include_http_headers`, and `files.extract`,
...
The concise schema compatibility contract is
