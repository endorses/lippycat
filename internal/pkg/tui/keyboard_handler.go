...
func (m Model) handleBridgeStats() (Model, tea.Cmd) {
	...
	msg := fmt.Sprintf("SIP:%d Mark:%d | Loss cap-gap:%d/%dB flush-gap:%d/%dB queue:%d/%dB parser:%d recovery:%d/%d | Det SDP:%d RTP Reg:%d RR:%d | TCP Req:%d+SDP:%d Rsp:%d+SDP:%d | RTP D:%d S:%d F:%d | Mrg S:%d A:%d OK:%d T:%d",
		tcpStats.SIPMessagesDetected,
		stats.TCPSIPFlowsMarked,
		stats.ReassemblyNormalDiscontinuities, stats.ReassemblyNormalMissingBytes,
		stats.ReassemblyExplicitFlushDiscontinuities, stats.ReassemblyExplicitFlushMissingBytes,
		tcpStats.PostReassemblyDroppedChunks, tcpStats.PostReassemblyDroppedBytes,
		tcpStats.ParserFramingDiscontinuities,
		tcpStats.RecoverySuccesses, tcpStats.RecoveryFailures,
		detMediaPorts,
	...
	logger.Debug("SIP correlation stats",
		...
		"reassembly_normal_discontinuities", stats.ReassemblyNormalDiscontinuities,
		"reassembly_normal_missing_bytes", stats.ReassemblyNormalMissingBytes,
		"reassembly_explicit_flush_discontinuities", stats.ReassemblyExplicitFlushDiscontinuities,
		"reassembly_explicit_flush_missing_bytes", stats.ReassemblyExplicitFlushMissingBytes,
		"post_reassembly_dropped_chunks", tcpStats.PostReassemblyDroppedChunks,
		"post_reassembly_dropped_bytes", tcpStats.PostReassemblyDroppedBytes,
		"parser_framing_discontinuities", tcpStats.ParserFramingDiscontinuities,
		"stream_recovery_successes", tcpStats.RecoverySuccesses,
		"stream_recovery_failures", tcpStats.RecoveryFailures,
