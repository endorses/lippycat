...
func BindGPUViperFlags(cmd *cobra.Command) {
	_ = viper.BindPFlag("hunter.voip_filter.enabled", cmd.PersistentFlags().Lookup("enable-voip-filter"))
	_ = viper.BindPFlag("hunter.voip_filter.gpu_backend", cmd.PersistentFlags().Lookup("gpu-backend"))
	_ = viper.BindPFlag("hunter.voip_filter.gpu_batch_size", cmd.PersistentFlags().Lookup("gpu-batch-size"))
}
