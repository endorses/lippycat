...
func (bc *BoundedCounter) Get(key string) int {
	return bc.counts[key]
}

// GetAll returns all counts as a map
func (bc *BoundedCounter) GetAll() map[string]int {
	// Return a copy to prevent external modification
	result := make(map[string]int, len(bc.counts))
...
func (bc *BoundedCounter) Clear() {
	bc.counts = make(map[string]int, bc.capacity)
}
